import fakeIdentity from 'fake-identity';
let fake;
    ...
    preFillDevelopment() {
        ...
        if (Meteor.isDevelopment) {
            ...
            if (state === 'signUp') {
                switch (this._id) {
                    case 'name':
                        // name field is first in the form, let's generate it here
                        // because we want to use same fake identity also for email
                        fake = fakeIdentity.generate();
                        result = `${fake.firstName} ${fake.lastName}`;
                        break;
                    case 'email':
                        result = fake.emailAddress;
                        break;
                    case 'password': {
                        result = 'qwerty';
                        break;
